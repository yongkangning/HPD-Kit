...
plot_coverage=$8
bin_path=$9
bwt2_pathogen_thread=${10}
blast_pathogen_thread=${11}
...
# blast verification
bowtie2 -p ${bwt2_pathogen_thread} --quiet --omit-sec-seq --no-discordant --no-unal \
...
seq 2 | rush  -k -j 2 \
  """
    blastn -num_threads ${blast_pathogen_thread} -query ${sampleId}.${pathogen_type}.${taxid}.R{}.fa \
    -db ${pathogen_db_path}/${pathogen_type}/blast_index/${taxid} \
    -evalue 1e-5 \
    -max_target_seqs 1 \
    -outfmt '6 qseqid sseqid  pident length mismatch gapopen qstart qend sstart send evalue bitscore' \
    -out ${sampleId}.${pathogen_type}.${taxid}.R{}.blast.txt
  """
